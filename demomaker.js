...
bpmInput.addEventListener('change', () => {
    ...
    const oldBpm = bpm; // 1. Megjegyezzük a régi tempót!
    bpm = val;          // 2. Beállítjuk az újat
    
    // 3. Klipek újrapozicionálása (Zenei időhöz rögzítés)
    document.querySelectorAll('.audio-clip').forEach(clip => {
        const startSec = parseFloat(clip.dataset.start);
        
        // Kiszámoljuk, hányadik negyednél (beat) kezdődött a régi tempó szerint
        const beatPosition = startSec / (60 / oldBpm);
        
        // Kiszámoljuk, mennyi másodpercnél van ez az ütés az új tempó szerint
        const newStartSec = beatPosition * (60 / bpm);

        // Frissítjük a klip adatait és a vizuális bal (left) pozícióját
        clip.dataset.start = newStartSec;
        clip.style.left = `${newStartSec * PX_PER_SECOND}px`;
    });

    // 4. Bónusz: A Playhead (lejátszófej) is maradjon a megfelelő ütemen!
    const playheadBeat = currentPlayTime / (60 / oldBpm);
    currentPlayTime = playheadBeat * (60 / bpm);
    startOffset = currentPlayTime; // Hogy lejátszáskor is jó helyről induljon

    // 5. Rács és vonalzó újrarajzolása
    drawRuler();
    drawAllGrids();
